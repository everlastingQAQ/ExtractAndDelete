# Extract & Delete 4.1 Developer Preview 验收说明

本版本是 `4.1.0` / package `4.1.0.0` 的 Windows 11 x64 Developer Preview。交付资产只有一个未签名的 Inno Setup EXE 和同名 SHA-256 文件。它需要 Developer Mode，不提供 Store、MSIX、MSI、AppInstaller、便携版或自动更新。
...
```text
artifacts\release\4.1.0\
├─ ExtractAndDelete-Setup-4.1.0-x64.exe
└─ ExtractAndDelete-Setup-4.1.0-x64.exe.sha256
```
...
```powershell
Start-Process .\artifacts\release\4.1.0\ExtractAndDelete-Setup-4.1.0-x64.exe -Wait
.\scripts\verify-dev-install.ps1
```
...
InstallLocation ...\Programs\ExtractAndDelete\app-4.1.0.0
```
...
1. 已有仓库注册的 `4.0.0.0`：运行 4.1 EXE，确认新 package 注册成功，旧仓库目录和源码完全不变。
2. 已有同版本 4.1：再次运行 EXE，确认进入修复，不产生第二个 package 或第二个完整卸载项。
3. 已有高于 4.1 的 package：确认安装器在任何 package 变更前拒绝降级。
...
发布工作流只接受 `v4.1.0` tag，并验证 tag commit 属于 `origin/main`、版本配置一致、测试通过、Inno 6.7.3 和构建证明可用。公开 Release 必须：

- 标题为 `Extract & Delete 4.1.0 Developer Preview`。
...
gh attestation verify .\ExtractAndDelete-Setup-4.1.0-x64.exe --repo everlastingQAQ/ExtractAndDelete
```

再用下载后的 EXE 在干净 Windows 11 x64 VM 完成一次安装、菜单、真实解压、回收和完整卸载验收。

CLI 源码保留在 `src\ExtractAndDelete.Cli` 作为冻结历史，不进入 V4.1 安装器、默认构建、测试、发布或使用文档。
