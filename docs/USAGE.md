# Extract & Delete 4.1.2 使用文档

`Extract & Delete 4.1.2 Developer Preview` 是一个面向 Windows 11 x64 的单 EXE 安装版本。它仍需要 Windows Developer Mode；安装器未使用生产 Authenticode 签名，因此 SmartScreen 可能显示“未知发布者”警告。4.1.2 延续 4.1.1 的启动修复，并在保留自包含运行时的前提下只打包简体中文卫星资源。
...
1. 在 GitHub Release 下载 `ExtractAndDelete-Setup-4.1.2-x64.exe`。`.sha256` 文件是可选的完整性校验文件。
2. 可选地在 PowerShell 中校验：

   ```powershell
   Get-FileHash .\ExtractAndDelete-Setup-4.1.2-x64.exe -Algorithm SHA256
   Get-Content .\ExtractAndDelete-Setup-4.1.2-x64.exe.sha256
   ```
...
预期为 `Name=ExtractAndDelete`、`Version=4.1.2.0`、`Status=Ok`，安装目录末尾为 `app-4.1.2.0`。

## 3. 升级、修复和迁移

升级没有自动更新。下载更高版本安装器并再次运行即可。

- 已安装更低版本时，安装器先写入新 `app-4.1.2.0`，验证 payload 后再切换 package 注册。
- 如果旧版本的 `InstallLocation` 为空、目录丢失或清单仍有 `$targetnametoken$` 占位符，4.1.2 会将其识别为损坏残留并只修复固定的 ExtractAndDelete package。
...
确认 Developer Mode 已开启，检查 package 的 `Status=Ok`、`Version=4.1.2.0` 和非空 `InstallLocation`，然后重启 Explorer。重新运行 4.1.2 安装器可执行修复。
...
开发者构建、测试和发布流程见：[项目总设计](PROJECT_DESIGN.md)、[验收说明](ACCEPTANCE.md)、[4.1.2 Release Notes](releases/v4.1.2.md)、[4.1.1 历史 Release Notes](releases/v4.1.1.md)、[4.1.0 历史 Release Notes](releases/v4.1.0.md)、[第三方声明](../THIRD-PARTY-NOTICES.md)。CLI 源码仍保留作历史参考，但已冻结，不属于用户交付。
